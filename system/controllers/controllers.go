...
func (t Controller) Test(c *gin.Context) {
	...
	c.String(http.StatusOK, string("Hello"))
}

// DNSQuery hi.
func (t Controller) DNSQuery(c *gin.Context) {
	// h := core.NewH()
	var dnsQuery string
	...
	if checkDNS == "application/dns-message" {
		...
		if err != nil {
			log.Println("Encoding DNS packer failed")
			c.String(http.StatusInternalServerError, "Encoding DNS packer failed")
			return
		}
